...
/// Template structure for static array
/// This structure represents a static array with size known at compile-time or runtime.
...
public:
    ...
    StaticArray(T *__restrict__ arrayData, Size arraySize)
        : arrayData(arrayData), arraySize(arraySize)
    {
    }
    ...
    /// Creates a static array on GPU and copies the data to it
...
public:
    ...
    DynamicArray(T *__restrict__ arrayData, Size arraySize)
        : StaticArray<T, Size>(arrayData, arraySize), allocatedSize(arraySize)
    {
    }
    DynamicArray(decltype(nullptr))
        : StaticArray<T, Size>(nullptr, 0), allocatedSize(0)
    {
    }
    DynamicArray(StaticArray<T, Size> array)
        : StaticArray<T, Size>(array), allocatedSize(array.size())
    {
    }
